...
- 要判断 2026-09-09: 「3人同時発話禁止は厳しすぎるか」に**逆向きの証拠**。RESPOND のパイロットでユーザーは割り込み積極性を低め（0.2〜0.4）に設定するのを好んだ（台帳 2026-09-09 arxiv 2603.21682 行）。2026-09-08 の callsphere 行（軽微な重なりは健全）と対立するので、緩める判断は保留すべきかもしれない。
- 要判断 2026-09-09: turn-taking を「発話／相槌／沈黙」の**3クラス**として明示的に持つか（同上 F1=0.902）。現行は「喋る／喋らない」の2択で、沈黙が「何もしない」の残りカスになっている。沈黙を独立した行動として持てば、相槌の要判断（2026-09-07）と自発発話45〜90秒の設計を同じ枠で決められる。
- 要判断 2026-09-09: 「宛先でない発話に黙る」（採用3）を**前段の判定器として実装する**根拠が T1 論文になった（台帳 2026-09-09 arxiv 2510.22610 行）。論文は、現行の音声エージェントが全発話を自分宛て候補として扱うため人間側が話し方を変えて合わせている、と指摘する。配信の横という多人数・多音源の場では effect が大きいはずで、プロンプトでなく前段で落とす設計を優先度上げてよいか。
- 要判断 2026-09-10: 相槌の要判断（2026-09-07）に**両側の一次証拠**が揃った。全二重で相槌を常時入れる GPT-Live は、当事者が「最初の数分は『おぉ！』でも数分後には『くどい』」「黙ることを知らない」と書く一方（台帳 2026-09-10 の fresh_sedum5880 の2行）、好評側が挙げた具体的な挙動は相槌そのものではなく「まだ続きがありそうだな、と黙って待つ」だった（同 subarasikiai 行）。**評価を分けているのは相槌の有無ではなく「待てるか」**である可能性が高い。こん太だけ相槌を復活させる案（§2.9）より先に、沈黙を独立した行動として持つ案（要判断 2026-09-09）を決める順序が正しいかもしれない。
- 要判断 2026-09-11: 相槌の要判断（2026-09-07 / 2026-09-10）に**量的な答え**が出た。日本語話者35名の被験者内実験で、相槌・頷きは「有り」が注意深さ・実在感・共在感を有意に上げる一方、注意深さは相槌の**数に対して逆U字**（二次項 p=.009）で中間量が最大だった（台帳 2026-09-11 arxiv 2608.19527 の2行）。つまり「相槌なし」も「常時相槌」も外れで、争点は有無ではなく**量**になる。さらに相槌の有無で発話長と会話時間は変わらなかった＝相槌が効くのは「そばにいる感」であって滞在時間ではない（禁止5と整合）。§2.9 の「役ごとに密度を変える」案（こん太だけ短い相槌）は、この逆U字と最も整合する形なので、先に決めてよいかもしれない。
