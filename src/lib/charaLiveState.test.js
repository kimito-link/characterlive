...
describe('⑤ 見た目の解決(画像パスまで)', () => {
  it('★合成済み1枚ではなく【土台/目/口の3枚】を返す', () => {
    ...
    expect(look.parts.face).toContain('face-base');
    expect(look.parts.eyes).toContain('eyes-');
    expect(look.parts.mouth).toContain('mouth-');
  });

  /*
   * ★パーツ方式にした一番の理由の固定(合成済みでは不可能だったこと)。
   *   合成済みは「表情×口」で焼き付いた8通りしか無く、blink の絵は口の状態ごとに
   *   別ファイルが要った。パーツなら目と口が独立なので、口を開けたまま瞬きできる。
   */
  it('★口を開けたまま目だけ blink にできる(合成済みでは出来なかった)', () => {
    const talking = resolveCharaLiveLook({
      charaId: 'rinku',
      mode: 'answer',
      timeMs: 0,
      modeStartedAtMs: 0
    });
    const blinkEyes = charaPartPaths('rinku', 'blink', true);
    // 口のパーツは表情に依存しない = 目だけ差し替えれば済む。
    expect(blinkEyes.mouth).toBe(charaPartPaths('rinku', 'smile', true).mouth);
    expect(blinkEyes.eyes).toContain('eyes-blink');
    expect(talking.parts.mouth).toBeTruthy();
  });

  it('どのモード・どの時刻でも【実在するパーツ】しか指さない', () => {
    // ★実ファイル名の集合と突き合わせる(綴りの思い込みで通してしまわないように)。
    const real = new Set(listCharaPartPaths());
    expect(real.size).toBe(21);
    for (const mode of ['idle', 'react', 'answer', 'thinking']) {
      for (const id of CHARA_LIVE_IDS) {
        for (let t = 0; t < 30000; t += 97) {
          ...
          for (const part of ['face', 'eyes', 'mouth']) {
            expect(real.has(look.parts[part]), `${mode}/${id}/${part}=${look.parts[part]}`).toBe(
              true
            );
          }
        }
...
describe('⑥ 状態遷移', () => {
  ...
  it('描画モデルは常に 3 体ぶん揃う', () => {
    ...
    for (const m of model) {
      expect(m.parts.face).toBeTruthy();
      expect(m.parts.eyes).toBeTruthy();
      expect(m.parts.mouth).toBeTruthy();
