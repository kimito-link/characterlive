 ...
 *   - 既存資産を作り直さない:
 *       表情/口の画像解決 → charaParts.js の charaPartPaths
 *         (★移植元は合成済み1枚を返していた。ここでは【土台/目/口の3枚】を返す=
 *         目と口が独立して動く。理由は charaParts.js 冒頭を読むこと)
 ...
 *   mouthOpen: boolean,
 *   parts: { face: string, eyes: string, mouth: string },
...
export function resolveCharaLiveLook(input) {
  ...
  return {
    ...
    expression,
    mouthOpen,
    // ★重ねる3枚(土台/目/口)。合成済み1枚ではないので、口パク中でも
    //   目だけを blink に差し替えられる(=喋りながら瞬きできる)。
    parts: charaPartPaths(charaId, expression, mouthOpen),
    float,
    tiltDeg
  };
}
