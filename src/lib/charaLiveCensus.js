 ...
 *   mountedInContainer: boolean, // body 直下でなく、ちゃんと器の中に居るか
...
export function collectCharaLiveCensus(doc) {
  ...
  /*
   * ★body 直下に置くと覆われる(移植元 2026-08-25 の実害):
   *   全画面要素と同じ z-index になったとき、同値は DOM 順で後勝ちになるため
   *   後から入る側に必ず負ける。器(親要素)の中に入れておけば、その器の重なりに従う。
   *   ★移植元は器を .nlsb-stage(会場)と名指ししていたが、それはあのリポ固有の名前。
   *     ここでは「body/html 直下ではないこと」だけを見る(寄生先を選ばない判定)。
   */
  const parent = el.parentElement;
  out.mountedInContainer = !!parent && parent !== doc.body && parent !== doc.documentElement;
  ...
  /*
   * ★覆われ検出: キャラの中心座標で最前面に居る要素を引き、自分(や自分の親)でなければ
   *   何かに覆われている＝見えていない、と判定する。移植元で3回外した原因が毎回これ。
   *
   * ★ただし素朴に elementFromPoint を使うと【必ず誤診する】(2026-09-04 実機で確認):
   *   ステージは pointer-events:none(下の操作を奪わないため)なので、
   *   ヒットテストの結果に【自分が一度も現れない】。移植元の実装をそのまま動かすと
   *   正常表示なのに「MAIN に覆われている」と赤を出した。
   *
   * ★そこで「自分が返るか」ではなく【自分の祖先が返るか】で見る:
   *   祖先が最前面にいる = その場所は自分の系統のものなので覆われていない。
   *   無関係な要素が返った = 本当に別のものが上に乗っている。
   */
  if (out.rect.w > 0 && typeof doc.elementFromPoint === 'function') {
    ...
    // top が自分自身/子孫/祖先のいずれでもなければ、本当に別物が覆っている。
    const mine = !top || top === el || el.contains(top) || top.contains(el);
    if (!mine) {
...
export function charaLiveVerdict(c) {
  ...
  if (!c.mountedInContainer) return ng(`body 直下に置かれている(親=${c.parent})=覆われる`);
